  ...
  useEffect(() => {
    if (subaccountId) {
      const fetchData = async () => {
        const response = await getSubAccountsTeamMembers(subaccountId)
        if (response) setAllMembers(response)
      }
      fetchData()
    }
  }, [subaccountId])
      ...
      if (defaultData.ticket.customerId) {
        ...
        const fetchData = async () => {
          const response = await searchContacts(
            // @ts-ignore
            //to fix it maybe we need to add || '' to the defaultData.ticket?.Customer?.name
